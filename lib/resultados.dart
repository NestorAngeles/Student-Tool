...
import 'package:proyecto_student_tool/final.dart';
import 'package:proyecto_student_tool/home.dart';

class Resultados extends StatefulWidget {

  Resultados({this.aprobadas,this.listacarrera,this.boolcarrera,this.dolar,this.ucredito,this.limiteuc});
  ...
  @override
  ResultadosState createState() => new ResultadosState(aprobadas: aprobadas, listacarrera: listacarrera, boolcarrera: boolcarrera, ucredito: ucredito, dolar: dolar, limiteuc: limiteuc);

  }

  class ResultadosState extends State<Resultados>{

    ResultadosState({this.aprobadas,this.listacarrera,this.boolcarrera, this.ucredito, this.dolar, this.limiteuc});
    var aprobadas = new List();
    var finales = new List();
    var listacarrera = new List();
    ...
    void initState() {
      ...
      print(resultados[0]);
      print('LIMITEEEE'+limiteuc.toString());
  ...
  Widget build(BuildContext context) {
    return new  Scaffold(
      appBar: AppBar(
        title: Text('UC= '+ acumuc.toString()+'/'+limiteuc.toString()),
        backgroundColor: Color(verde),
          actions: <Widget>[
            FlatButton(
                onPressed: (){
                  if(acumuc>14) {
                    Navigator.push(context,
                        ...
                        new Final(aprobadas: finales,
                          acumuc: acumuc,
                          acumbs: acumbs,
                          acumdolares: acumdolar,
                          ucredito: ucredito,
                          dolar: dolar,)));
                  }else{
                    Fluttertoast.showToast(
                        backgroundColor: Colors.red,
                        msg: 'Debe ingresar un minimo de 15uc',
                ...
                Row(
                    children: <Widget>[
                      Text('Siguiente',style: TextStyle(color: Colors.white,fontSize: 15)),
  ...
  void acumularuc(int i, bool value) {
    if (((acumuc + resultados[i]['UC']) > limiteuc) && value == true) {
      setState(() {
        checks[i] = false;
      });
      Fluttertoast.showToast(
          backgroundColor: Colors.red,
          msg: 'Esa materia excede las ' + limiteuc.toString() +
    ...
    if (!value) {
      print(acumuc);
      finales.remove(resultados[i]);
      acumuc = acumuc - resultados[i]['UC'];
      acumbs = acumbs - ((resultados[i]['UC'] * ucredito));
      acumdolar = acumdolar - ((resultados[i]['UC'] * ucredito) ~/ dolar);
    } else {
      if (value && (acumuc + resultados[i]['UC']) <= limiteuc) {
        finales.add(resultados[i]);
        acumuc = acumuc + resultados[i]['UC'];
        acumbs = acumbs + ((resultados[i]['UC'] * ucredito));
        acumdolar = acumdolar + ((resultados[i]['UC'] * ucredito) ~/ dolar);
      }
